...
public:
    ...
    void boostPower(int x) {
        // boost by actor i.r *x
        (*this) * (x);
    }
    void boostPower(qns suit2) {
        // boost using another suit i.e +suit2
        (*this) + suit2;
    }
    ...
    bool is_sd() {
        // bool function to check is suit shut down
        return h > 500;
    }
    bool is_destroyed(){
         // bool function to check is suit destroyed
        return d <= 0;
    }
};
class avenger{
...
public:
    // all battle simulation is done here
    void startBattle() {
        int k, n, m;
        cin >> k >> n >> m;
        ...
        // input for n avengers
        for (int i = 0; i < n; ++i) {
            string name;
            cin >> name;
            int attackstrength;
            cin >> attackstrength;
            // check if is suit is available
            if (!suits.empty()) {
    ...
    int Result(){
        ...
        for (auto i = 0; i < heroes.size(); ++i){
            // add only heroes that are not destroyed
            if (!heroes[i].suit.is_destroyed()) {
                hero_sum += (heroes[i].suit.get_p() + heroes[i].suit.get_d());
            }
        }
        for (auto i = 0; i < enemies.size(); ++i){
            // add only enemies that are not destroyed
            if (!enemies[i].suit.is_destroyed()) {
